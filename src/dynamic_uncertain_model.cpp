...
public:
	...
	void time_step()
	{
		...
		Yv = 0.5*(-40*1000*abs(nu_r(1)))*(1.1+0.0045*(1.01/0.09)-0.1*(0.27/0.09)+0.016*(pow((0.27/0.09),2)))*(3.141592*0.09*1.01/2);
		...
		// ROS_INFO_STREAM("M: " << M.inverse() );
		// ROS_INFO_STREAM("Dl: " << Dl);
		// ROS_INFO_STREAM("Dn: " << -Dn);
		// ROS_INFO_STREAM("C: " << CRB+CA);
...
{
	ros::init(argc, argv, "dynamic_uncertain_model");
